...
def CKD():
    if request.method == "POST":
        ...
        alb = str(float(submitted_values["albumin"].strip()))
        hem = str(float(submitted_values["hemoglobin"].strip()))
        pcv = str(float(submitted_values["pcv"].strip()))
        hyp = str(float(submitted_values["hypertension"].strip()))
        sc = str(float(submitted_values["sc"].strip()))

        ckd_inputs1 = [sg, alb, sc, hem, pcv, hyp]
        prediction = decisionTree.predict([ckd_inputs1])
        # print("**************             ", prediction)
        if not prediction:
            return render_template("Infected.htm", disease="Rheumatoid arthritis Disease")
        else:
            return render_template("NonInfected.htm")

    return render_template("arthritis.html", title="Arthritis Disease", navTitle="Arthritis Disease", headText="Arthritis Disease Detector", ImagePath="/static/Chronic_Kidney.png")
